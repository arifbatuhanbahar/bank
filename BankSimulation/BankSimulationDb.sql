-- =============================================
-- BANKA SIMULASYONU VERITABANI SEMASI
-- Temiz SQL Script (EF Core bagimliligi yok)
-- 38 Tablo - 9 Modul
-- =============================================

-- Guvenlik: Varsayilan olarak mevcut veritabani DROP edilmez.
DECLARE @ALLOW_DROP BIT = 0; -- 1 yaparsaniz DB drop edilir ve yeniden olusur.

IF @ALLOW_DROP = 1
BEGIN
    IF EXISTS (SELECT * FROM sys.databases WHERE name = 'BankSimulationDb')
    BEGIN
        PRINT 'Dropping existing BankSimulationDb (ALLOW_DROP = 1)...';
...
BEGIN
    IF EXISTS (SELECT * FROM sys.databases WHERE name = 'BankSimulationDb')
        PRINT ''WARNING: BankSimulationDb already exists. Skipping DROP (set @ALLOW_DROP = 1 to recreate).'';
END
GO

IF DB_ID('BankSimulationDb') IS NULL
BEGIN
    CREATE DATABASE BankSimulationDb;
    PRINT ''Created BankSimulationDb.'';
END
ELSE
BEGIN
    PRINT ''Using existing BankSimulationDb.'';
END
GO
